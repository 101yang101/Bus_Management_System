...
public class FleetCaptainView implements AutoCloseable{
    private final Connection connection;
    private int companyID;
    private String workNum;

    // 构造函数接收一个已有的数据库连接，并进行登录验证
    public FleetCaptainView(int companyID, String workNum) throws SQLException, DriverView.LoginFailedException {
        this.connection = DatabaseConnection.connect();
        this.companyID = companyID;
        this.workNum = workNum;

        if (!login()) {
            throw new DriverView.LoginFailedException("登录失败: 未找到该车队队长的信息");
        }
    }

    // 执行登录验证
    private boolean login() throws SQLException {
        final String QUERY_VERIFY_DRIVER = "SELECT 1 FROM Driver_View WHERE company_ID = ? AND driver_work_num = ? AND driver_job = '车队队长' FETCH FIRST 1 ROW ONLY";
    ...
    public void queryRouteCaptainInfo(int companyID, String workNum) throws SQLException {
        final String QUERY_DRIVER_INFO =
                "SELECT driver_name, company_name, driver_work_num, driver_ID, driver_job, driver_birthday, driver_phone, driver_fleet_ID, driver_gender " +
        ...
        try (PreparedStatement pstmt = connection.prepareStatement(QUERY_DRIVER_INFO)) {
            ...
            try (ResultSet rs = pstmt.executeQuery()) {
                if (rs.next()) {
                    ...
                    System.out.println("未找到该车队队长的信息");
                }
            }
        }
    }
    ...
    public void displayManagedDriversInTable() {
        String functionCall = "SELECT * FROM get_fleet_captain_drivers(?, ?)";
        ...
        try (PreparedStatement stmt = connection.prepareStatement(functionCall)) {
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                ...
                while (rs.next()) {
                    hasResults = true;
                    printTableRow(
                            rs.getInt("driver_route_ID"),
                            rs.getString("driver_job"),
    ...
    private void printTableRow(int driverRouteID, String driverJob, String driverName, String driverWorkNum, String carID, String driverPhone) {
        System.out.printf("%-15d | %-15s | %-20s | %-20s | %-20s | %-20s%n",
                driverRouteID, driverJob, driverName, driverWorkNum, carID, driverPhone);
    }
    ...
    public boolean isDriverManagedByRouteCaptain(int companyID, String workNum) {
        String sql = "SELECT driver_fleet_ID FROM Driver "
        ...
        if(companyID == this.companyID && driver_fleet_ID == route_captain_fleet_ID)
        {
            return true;
        }
        else{
    ...
    public boolean isDriverManagedByRouteCaptain(String driverID) {
        ...
        sql = "SELECT driver_fleet_company_ID, driver_fleet_ID FROM Driver "
                + "WHERE driver_ID = ?";
        ...
        if(driver_company_ID == this.companyID && driver_fleet_ID == route_captain_fleet_ID)
        {
            return true;
        }
        else{
    ...
    private void printFleetViolationStats(int fleetID, LocalDateTime startTime, LocalDateTime endTime) throws SQLException {
        ...
        StringBuilder summary = new StringBuilder("车队(" + fleetID + ")在指定时间段内");
        ...
        try (PreparedStatement pstmt = connection.prepareStatement(QUERY_FLEET_VIOLATION_STATS)) {
            ...
            try (ResultSet rs = pstmt.executeQuery()) {
                while (rs.next()) {
                    ...
                    summary.append(", ").append(violationName).append(": ").append(count).append("次");
                }
                if (!hasViolations) {
                    summary.append("无违章记录");
                } else {
                    summary.insert(summary.indexOf("内") + 2, "共有" + totalViolations + "次违章");
                }
                System.out.println(summary.toString().replaceFirst(", ", " "));
            }
        }
    }
    ...
    // 录入司机信息
    public boolean addDriver(String workNum, String name, char gender, String id, String phone, Date birthday, String carID, int route_ID, String job) {
        String sql1 = "SELECT driver_fleet_ID FROM Driver "
                + "WHERE driver_fleet_company_ID = ? AND driver_work_num = ? ";
        ...
        if(job.equals("路队长"))
        {
            String sql2 = "SELECT 1 FROM Driver "
                    + "WHERE driver_fleet_company_ID = ? AND driver_route_ID = ? AND driver_job = '路队长'";
            ...
            try (PreparedStatement stmt = connection.prepareStatement(sql2)) {
                ...
                try (ResultSet rs = stmt.executeQuery()) {
                    if (rs.next()) {
                        System.out.printf("该线路已经有路队长，不能重复设置!!!\n");
                        return false;
                    }
                }
            } catch (SQLException e) {
        ...
        String sql3 = "INSERT INTO Driver (driver_name, driver_fleet_company_ID, driver_fleet_ID, driver_ID, driver_work_num, driver_job, driver_birthday, driver_phone, driver_gender, driver_route_ID, driver_car_ID) "
                + "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

        try (PreparedStatement stmt = connection.prepareStatement(sql3)) {
            stmt.setString(1, name);
            stmt.setInt(2, companyID); // 关联到当前车队队长的公司
            ...
            stmt.setString(9, Character.toString(gender));
            stmt.setInt(10, route_ID);
